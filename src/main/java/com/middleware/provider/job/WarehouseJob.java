...
public class WarehouseJob implements Job {
    ...
    private static final org.apache.logging.log4j.Logger LOGGER = LogManager.getLogger(WarehouseJob.class);
        ...
        try {
            ...
            if (existingJobDetail != null) {
                if (enviromentConfig.isRunJobs()) {
                    ...
                    //this.syncWarehouses(jec.getFireTime());
                }
            }

        } catch (Throwable th) {
            LOGGER.error("WarehouseJob - FATAL ERROR", th);
     ...
     */
    private Boolean syncWarehouses(Date taskExecutionDate) {
        LOGGER.info("Sincronizando warehouses a VTEX");
        ...
        try {
            ...
            try {
                skuBs.searchWarehouse();
            } catch (Exception ex) {
                LOGGER.error("Error sincronizando warehouses generadas en VTEX", ex);
